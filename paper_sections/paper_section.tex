...
% WHAT TO WRITE:
% - Completeness (3.32/5) is the weakest dimension across all domains.
% - The model misses subtle or domain-specific aspects rather than hallucinating.
% - Root cause: training data covers only 5 restaurant-specific aspect categories;
%   out-of-domain aspects (foiz_stavka, tarif, dori) have no training signal.
% - This suggests that domain-adaptive fine-tuning or few-shot prompting with
%   domain-specific aspect taxonomies could substantially improve completeness.
% - The "boshqalar" category dominance (45.5%) further evidences this:
%   the model correctly identifies aspects but cannot assign domain-appropriate labels.

% ----------------------------------------------------------------------
\subsection{Architecture vs.\ Pre-Training Effects}
\label{sec:disc_arch_pretraining}
% WHAT TO WRITE:
% - Qwen vs DeepSeek: same Qwen-7B backbone, but DeepSeek's R1 reasoning
%   distillation does NOT help structured ABSA — it actually hurts.
%   Possible explanation: reasoning chains (<think> patterns) conflict with
%   direct JSON output generation.
% - Qwen vs Llama: different architectures. Llama achieves lower loss and
%   better sentiment accuracy, but Qwen produces more reliable structured output
%   (100% parse rate, better ATE recall).
% - Practical implication: for production annotation pipelines, instruction-following
%   reliability (JSON parse rate) may matter more than marginal metric differences.

% ----------------------------------------------------------------------
\subsection{Loss as a Model Selection Proxy}
\label{sec:disc_loss_proxy}
% WHAT TO WRITE:
% - The divergence between loss ranking and task ranking is a notable finding.
% - Cross-entropy loss captures token-level prediction quality but is blind to:
%   (a) JSON structural correctness, (b) aspect boundary precision,
%   (c) the distinction between extracting a correct vs incorrect aspect term
%   when both are plausible continuations.
% - This has implications beyond our work: researchers selecting models based
%   solely on validation loss may not find the best task performer.
% - Recommendation: always evaluate on task-specific metrics, especially for
...
% WHAT TO WRITE:
% - Single training domain: model trained only on restaurant reviews.
%   Domain-adaptive training is a natural extension.
% - Silver-standard annotations: no human gold-standard for the multi-domain corpus.
%   Human verification of 50 reviews is planned but not yet complete.
% - No zero-shot or encoder-based baselines (mBERT, XLM-R) for comparison.
% - LLM-as-Judge may have its own biases (positional bias, score inflation).
% - Class imbalance: positive polarity dominates both datasets.
% - Single evaluation split: val set used (90/10); 80/10/10 with held-out test
%   recommended for the camera-ready version.
% - No tokenizer fertility analysis to quantify each model's Uzbek token efficiency.
