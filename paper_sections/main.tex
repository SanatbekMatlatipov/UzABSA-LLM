...
\newcommand{\orcidauthorD}{0000-0002-7876-5009} % Add \orcidD{} behind the author's name

% Authors, for the paper (add full first names)
\Author{Sanatbek Matlatipov $^{1}$\orcidA{}, Khabibulla Madatov $^{2}$\orcidB{}, Jaloliddin Rajabov $^{1}$\orcidC{} and Jernej Vi\v{c}i\v{c} $^{3}$\orcidD{}}

%\longauthorlist{yes}

% MDPI internal command: Authors, for metadata in PDF
\AuthorNames{Sanatbek Matlatipov, Khabibulla Madatov, Jaloliddin Rajabov and Jernej Vi\v{c}i\v{c}}
...
\abstract{Aspect-Based Sentiment Analysis (ABSA) remains largely unexplored for Uzbek, a low-resource Turkic language with limited representation in current foundation models. This study presents, to the best of our knowledge, the first systematic investigation of parameter-efficient fine-tuning for Uzbek ABSA, comparing three open-source large language models---Qwen~2.5-7B, Llama~3.1-8B, and DeepSeek-R1-Distill-Qwen-7B---each evaluated under identical QLoRA configurations on a SemEval-format restaurant review dataset comprising 6,175 annotated sentences. ABSA is reformulated as a generation task with Uzbek-language instruction tuning, enabling models to jointly extract aspect terms and classify sentiment polarity in a single pass, producing structured JSON output. Evaluation on 609 validation examples demonstrates that Qwen~2.5-7B achieves the highest aspect term extraction F1 score of 0.6603 with a perfect 100\% JSON parse rate, while Llama~3.1-8B leads in end-to-end pair F1 (0.5805) and sentiment accuracy (0.8864). Importantly, training loss does not reliably predict downstream task performance: Qwen surpasses Llama in extraction despite a 31\% higher validation loss. Based on these findings, a three-layer model-assisted annotation pipeline is proposed, integrating local inference, LLM-as-Judge quality scoring, and quality-tiered assembly. Application of this pipeline to 5,038 multi-domain Uzbek business reviews across 23 categories produces a novel silver-standard dataset containing 9,884 aspect–sentiment pairs, with 63.5\% of judged annotations meeting inclusion quality thresholds.}
...
\item \textbf{A novel multi-domain silver-standard Uzbek ABSA dataset} covering 5{,}038 reviews across 23~business categories with 9{,}884 annotated aspect--sentiment pairs and per-annotation quality metadata, together with a domain-specific aspect sub-category taxonomy of 119 fine-grained sub-categories across 18 business domains (e.g., \textit{kredit}, \textit{foiz\_stavka} for banking; \textit{shifokor\_malakasi}, \textit{diagnostika} for healthcare) designed to extend the flat 5-label restaurant taxonomy to specialised domains.
...
\informedconsent{Not applicable. The study did not involve human or animal subjects; the analysed reviews are publicly available user-generated content collected with the explicit permission of the platform owner (see \Cref{sec:acknowledgement}).}
