...
As required, we disclose that an earlier version of this manuscript was submitted to \textit{Computers} (MDPI) and was not accepted; the previous manuscript ID is supplied in the submission system. The present version differs substantially: all models were retrained and re-evaluated after we identified and corrected two defects in the earlier pipeline (a system-prompt serialization bug and text-level overlap between the training and evaluation splits), and we added zero-shot controls, encoder baselines, paired-bootstrap significance testing across all system pairs, and a native-speaker validation study. The reported results are new, and the claims have been narrowed to what the corrected experiments support.

The author list has also changed relative to that submission: Gusein Djalilov joined the work and contributed to validation, formal analysis, and revision of the manuscript, while Abdullah M.\ Almarashi is no longer an author and has agreed to this change. The remaining authors are unchanged. All four current authors have seen and approved this version, and individual contributions are itemised in the manuscript's Author Contributions statement.
