...
\abstract{In this paper, we investigate the application of parameter-efficient fine-tuning to scalable aspect-based sentiment analysis (ABSA) of Uzbek business reviews. Specifically, we fine-tune Qwen 2.5-7B, Llama 3.1-8B, and DeepSeek-R1-Distill-7B with the same QLoRA setting to jointly extract aspect terms and classify the polarity of the corresponding aspects in a structured JSON output format. Evaluation is done on a dataset consisting of 609 review-grouped and deduplicated splits, along with comparison with zero-shot baselines. In fine-tuned settings, nearly doubling the F1 for aspect term extraction and tripling the F1 for aspect polarity pairs were observed. Qwen obtains the highest extraction F1 score (0.708) and 100\% parsing rate, whereas Llama is better at predicting the pair F1 (0.651) and sentiment accuracy (0.925). We further benchmark the fine-tuned LLMs against Uzbek BERT encoders fine-tuned with 110M parameters. While LLMs show a better result in sentiment classification and joint prediction, the two model types have comparable results in aspect extraction. As the next step, we describe a three-step annotation pipeline: local LLM annotation, LLM-as-Judge quality evaluation, and data assembly based on quality scores. The pipeline is applied to 5,038 business reviews covering 23 domains, which results in 9,884 aspect-level annotations. Quality scores by native speakers acting as benchmark judges are positively correlated with human ratings with  $\rho=0.66$. Moreover, we provide a reconciled and double-annotated dataset of 80 reviews. All code, models, and datasets are available.}
...
The paper has been constructed based on three research questions. The first one concerns the possibility of carrying out joint aspect extraction and sentiment classification for Uzbek through parameter-efficient fine-tuning with QLoRA for an open source large language model. The second research question is about analyzing the selected models within the same QLoRA tuning setting to see if the validation loss can be considered as an indicator of the model performance in downstream tasks. The third research question addresses the possibility of annotating data from businesses other than restaurants by a model trained only on restaurant data.

In addition to specific language-based issues, the current research deals with the problem of big textual analysis, which involves the issue of turning an open-ended flow of texts produced by users in a continuous manner into a high-quality resource for analysis in a situation where manual annotation does not match the speed of generation of texts. There are more texts generated by review websites than can be annotated by people. Additionally, in low-resource languages, there are no annotators available to do that task. Our solution is a combination of a locally fine-tuned annotator and an automatic cognitive evaluation layer, which is an LLM judge, calibrated against native-speaker judgement and a tiered assembly step that makes annotation quality explicit per instance in the released data. Our pipeline is model-agnostic and can be generalized to other low-resource languages and domains suffering the same annotation bottleneck.
...
\item The proposed ABSA dataset for Uzbek spans five domains, including 5{,}038 reviews from 23 business domains, with 9{,}884 aspect-sentiment pairings. For the present experiments, we use the initial five categories for labels in the UzABSA task; an additional resource of 119 subcategory taxonomies is provided for future possible domain-specific annotations.
\end{enumerate}

\noindent It can be noted that in all the experiments, there is one observation which is common, and it is that the cross entropy loss minimization is not in line with the model's effectiveness in performing the ABSA task, i.e., the model which has the least loss is not the best one for the extraction of aspects\Cref{sec:discussion}.
