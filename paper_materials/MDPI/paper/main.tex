...
\abstract{We present a controlled study of parameter-efficient fine-tuning of open-weight large language models (LLMs) for Uzbek aspect-based sentiment analysis (ABSA). Qwen~2.5-7B, Llama~3.1-8B, and DeepSeek-R1-Distill-7B are fine-tuned under identical QLoRA configurations to jointly extract aspect terms and assign their sentiment as structured JSON, and evaluated on a 609-example review-grouped, deduplicated split against zero-shot controls. Fine-tuning at least doubles exact extraction F1 and triples aspect--polarity pair F1: Qwen achieves the best extraction F1 (0.708) with a 100\% parse rate, while Llama leads on pair F1 (0.651) and sentiment accuracy (0.925); paired-bootstrap tests detect no significant difference between the two. Benchmarking against fine-tuned 110M-parameter Uzbek BERT encoders reveals a task-dependent trade-off: no significant difference is detected at aspect extraction, whereas the LLMs are clearly stronger at sentiment and joint prediction. We then propose a three-layer model-assisted annotation pipeline---local LLM annotation, LLM-as-Judge quality scoring, and quality-tiered assembly---and apply it to 5{,}038 business reviews from 23 domains, producing a multi-domain Uzbek ABSA dataset with 9{,}884 aspect-level annotations. A native-speaker validation study calibrates the judge against human ratings (Spearman $\rho=0.66$ overall) and contributes an 80-review reconciled, double-annotated subset. All code, models, and datasets are publicly released.}
...
We use LLM-as-Judge for a different goal: assessing structured ABSA annotations produced by a fine-tuned model in a low-resource setting without gold references. Concretely, GPT-4o-mini scores Uzbek aspect--sentiment outputs on five rubric dimensions (completeness, accuracy, sentiment correctness, relevance, overall quality) using a 1--5 Likert scale. Unlike prior work, our target is structured extraction rather than free-form text, and the judge must understand Uzbek input; we then apply stratified sampling over 23 domains and quality thresholds to guide dataset assembly (\Cref{sec:meth_pipeline}). Closest to our pipeline, Hellwig et al.~\cite{hellwig2026llmannotator} train lightweight ABSA models directly on LLM-generated annotations, establishing model-assisted annotation as a viable route for ABSA; our pipeline differs in adding an explicit judging layer whose scores are themselves calibrated against native-speaker ratings (\Cref{sec:res_human}).
...
% ----------------------------------------------------------------------
\textbf{When is a 7B LLM Worth It?} The encoder baselines sharpen the practical picture. A 110M monolingual encoder, fine-tuned in ten minutes on a laptop, shows no statistically significant difference from the 7--8B LLMs at aspect-term extraction---a reminder that for span identification in a morphologically rich language, targeted monolingual pre-training (BERTbek, TahrirchiBERT) remains remarkably effective. The LLMs justify their cost where the task is genuinely joint and generative: sentiment macro-F1 (+5--7 points over the encoders), unified term- and category-level output in a single generation, and the ability to follow natural-language instructions on unseen domains, which is what the multi-domain annotation pipeline exploits. Teams building Uzbek ABSA systems under tight compute budgets should therefore consider encoders for extraction-only workloads and reserve LLMs for joint prediction and annotation-generation scenarios.

% ----------------------------------------------------------------------
\textbf{Cost of the Pipeline}. Because the practical case for model-assisted annotation rests on its economics, we report them explicitly. All QLoRA fine-tuning ran on a single 48~GB NVIDIA RTX~A6000: 1{,}000 optimizer steps took 35~min (Qwen), 48~min (Llama), and 84~min (DeepSeek) of pure training time, or 65--107~min of wall clock including periodic evaluation. Annotating the 5{,}038-review multi-domain corpus with the fine-tuned Qwen model (Layer~1) ran at a median 4.3~s per review---roughly 840 reviews per hour---for approximately 6~GPU-hours end to end. Layer~2 judging is cheaper still: the 307-review stratified sample cost approximately \$0.05 in \texttt{gpt-4o-mini} API charges, which extrapolates to under \$1 to judge the corpus in full. The dominant remaining cost is therefore human: the validation study in \Cref{sec:res_human} required two native speakers to rate 150 reviews and to annotate 80 from scratch, and it is this step---not GPU time or API spend---that bounds how much of a corpus can be verified. That asymmetry is the practical argument for the pipeline: machine annotation and machine judging are inexpensive enough to apply exhaustively, so scarce native-speaker effort can be spent on calibrating and spot-checking the automatic layers rather than on labeling every review.
