...
Syntactic annotation has begun to catch up with this morphological groundwork. A Universal Dependencies treebank of literary and educational Uzbek text now supplies gold morphosyntactic and dependency annotation for the language~\cite{MATLATIPOV2026112857}, and parsing studies built on it compare graph-based against transition-based architectures over BERT-based encoders~\cite{11632281} and show that cross-treebank training improves robustness when any single Uzbek treebank remains small~\cite{10.1007/978-3-032-29532-3_15}. This matters for ABSA because aspect terms in an agglutinative language are frequently nominal heads carrying case and possessive suffixes, and their opinion targets are identified by dependency relations rather than by adjacency---which is precisely the signal that syntax-aware ABSA taggers exploit~\cite{Yusufu2026}. The models compared in this study operate on raw text and use none of this structure; the availability of these resources is what would make syntax-aware Uzbek ABSA practical, and we return to it as future work in \Cref{sec:conclusion}.
