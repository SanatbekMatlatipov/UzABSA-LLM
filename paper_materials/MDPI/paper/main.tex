...
\newcommand{\orcidauthorB}{0009-0007-3089-0867} % Add \orcidB{} behind the author's name
\newcommand{\orcidauthorC}{0000-0001-5207-8852} % Add \orcidC{} behind the author's name

% Authors, for the paper (add full first names)
\Author{Sanatbek Matlatipov $^{1,}$*\orcidA{}, Gusein Djalilov $^{2}$\orcidB{} and Mersaid Aripov $^{1}$\orcidC{}}

%\longauthorlist{yes}

% MDPI internal command: Authors, for metadata in PDF
\AuthorNames{Sanatbek Matlatipov, Gusein Djalilov and Mersaid Aripov}

%\longauthorlist{yes}

% Affiliations / Addresses (Add [1] after \address if there is only one affiliation.)
\address{%
$^{1}$ \quad Faculty of Applied Mathematics and Intelligent Technologies, National University of Uzbekistan named after Mirzo Ulugbek, Tashkent 100174, Uzbekistan; s.matlatipov@nuu.uz (S.M.); mirsaid.aripov@nuu.uz (M.A.)\\
$^{2}$ \quad Independent Researcher, Tashkent, Uzbekistan; g.djalilov@gmail.com\\
}
...
\abstract{Uzbek, a Turkic language with almost 38 million speakers, remains under-served in aspect-based sentiment analysis (ABSA). We present, to our knowledge, the first systematic study of parameter-efficient fine-tuning of open-source large language models (LLMs) for Uzbek ABSA: Qwen~2.5-7B, Llama~3.1-8B, and DeepSeek-R1-Distill-7B are fine-tuned under identical QLoRA configurations to jointly extract aspect terms and classify sentiment as structured JSON. On a 609-example evaluation set, Qwen achieves the best LLM extraction F1 (0.660) with a 100\% parse rate, while Llama leads in end-to-end pair F1 (0.581) and sentiment accuracy (0.886). Benchmarking against fine-tuned Uzbek BERT encoders reveals a task-dependent trade-off: a 110M-parameter encoder slightly surpasses the LLMs at span extraction, whereas the LLMs are clearly stronger at sentiment and joint prediction. We then propose a three-layer model-assisted annotation pipeline---local inference, LLM-as-Judge quality scoring, and quality-tiered assembly---and apply it to 5{,}038 business reviews from 23 domains, producing a multi-domain Uzbek ABSA dataset of 9{,}884 aspect--sentiment pairs. A native-speaker validation study calibrates the LLM judge against human ratings (Spearman $\rho=0.66$ for overall quality) and contributes an 80-review gold-verified subset. All code, models, and datasets are publicly released.}

% Keywords
\keyword{Aspect-Based Sentiment Analysis; Low-Resource NLP; Uzbek Language; Parameter-Efficient Fine-Tuning; QLoRA; Large Language Models; LLM-as-Judge; Multi-Domain Dataset}
...
Aspect-Based Sentiment Analysis (ABSA) identifies specific aspects and their polarities in text \cite{pontiki-etal-2014-semeval}. While high-resource languages dominate the field, Uzbek---a Turkic language with almost 38 million speakers\footnote{\url{https://www.worldometers.info/world-population/uzbekistan-population/}}---remains comparatively under-explored, owing to script duality (Latin/Cyrillic), agglutinative morphology, and limited representation in multilingual pre-training corpora. Recent 7--8B-parameter LLMs such as Qwen~2.5~\cite{Qwen2024-kr}, Llama~3.1~\cite{Grattafiori2024-ka}, and DeepSeek-R1~\cite{DeepSeek-AI2026-vt} offer cross-lingual transfer potential. Using QLoRA~\cite{10.5555/3666122.3666563}, we adapt these models on consumer hardware by reformulating ABSA as a single-pass JSON generation task.

This work is guided by three questions. First, we ask to what extent parameter-efficient fine-tuning with QLoRA enables open-source LLMs to perform joint aspect extraction and sentiment classification for Uzbek. Second, we examine how the chosen architectures compare under identical QLoRA configurations, and whether validation loss is a reliable guide to downstream task performance. Third, we ask whether a model trained only on restaurant reviews can produce usable annotations across a wider range of business domains, and how quality varies with distance from the training domain.

Beyond the language-specific results, the study addresses a problem generic to large-scale text analytics: turning a heterogeneous stream of user-generated reviews into a quality-controlled analytical resource when expert annotation cannot scale. Our answer couples a locally fine-tuned annotator with an automated cognitive evaluation layer---an LLM judge whose scores we calibrate against native-speaker judgement---and a tiered assembly step that makes annotation quality explicit, per instance, in the released data. The pipeline is model-agnostic and transfers to other low-resource languages and domains where the same annotation bottleneck applies.

\noindent In addressing these questions, we make the following contributions:

\begin{enumerate}
\item To our knowledge, the first systematic evaluation of QLoRA-fine-tuned open-source LLMs for Uzbek ABSA, comparing three architectures at the same parameter scale under identical training conditions, and benchmarked against fine-tuned Uzbek BERT encoders.

\item A scalable model-assisted annotation pipeline that combines local inference, LLM-as-Judge quality scoring, and quality-tiered assembly, together with a native-speaker validation study (a two-stage annotation-agreement analysis, calibration of the LLM judge, and a gold-verified subset) that quantifies the reliability of the resulting resource.
...
\end{equation}
in BFloat16, reducing the memory footprint of a 7B-parameter model to under 6~GB while leaving gradient flow through the full-precision adapters intact~\cite{10.5555/3666122.3666563}.
...
\caption{The three-layer model-assisted annotation pipeline. A locally fine-tuned LLM annotates the full multi-domain corpus; an external LLM judge scores a stratified sample on a five-dimension rubric; assembly partitions the corpus into quality tiers. A native-speaker validation study (dashed) calibrates the judge and contributes a gold-verified subset.}
\label{fig:pipeline}
\end{figure}
...
\end{equation}
and $\mathrm{F1}_\mu = 2\,\mathrm{P}_\mu \mathrm{R}_\mu / (\mathrm{P}_\mu + \mathrm{R}_\mu)$. Micro-averaging across all test examples avoids bias from examples with few or many aspect terms.
...
\textbf{Sentiment Classification.} For aspect terms that are correctly matched (exact match), we additionally report sentiment accuracy and macro-averaged F1 over the $K{=}4$ polarity classes,
\begin{equation}
\label{eq:macrof1}
...
\end{equation}
Pairing by review preserves the per-example correlation between systems and therefore tests the difference rather than the two absolute scores.
...
% ----------------------------------------------------------------------
All experiments run on a single NVIDIA RTX A6000 GPU (48\,GB VRAM) using Python~3.10 with Unsloth, TRL, PEFT, and bitsandbytes. The UzABSA dataset is split 90/10 into training (5{,}480) and validation (609) sets with a fixed seed of~42. Each of the three models is trained for exactly 1{,}000 gradient steps under the QLoRA configuration described in \Cref{sec:meth_qlora}; the \emph{only} variable across runs is the base model. We use the term validation set because the processed split was originally prepared for controlled model comparison; no additional hyperparameter search was performed on this split after observing downstream ABSA metrics. A fully separate held-out test split and zero-/few-shot prompting baselines are left for future work; fine-tuned encoder baselines are evaluated on the same split (\Cref{sec:meth_encoder}). \Cref{tab:train_efficiency} summarises the resulting training efficiency.

\begin{table}[t]
\centering
\caption{Training efficiency on a single RTX A6000. All runs use identical QLoRA hyperparameters; the adapter size differs slightly with backbone geometry (40.4\,M trainable parameters for the Qwen-architecture models, 41.9\,M for Llama~3.1-8B).}
...
\begin{table}[t]
\centering
\caption{ABSA evaluation on the 609-example evaluation set. Best scores per metric are \textbf{bold}. LLMs use greedy decoding (\texttt{temperature}$=$0.1); encoder baselines (BERTbek, TahrirchiBERT) use BIO-polarity token classification (\Cref{sec:meth_encoder}) and produce structured output by construction, so the JSON parse rate does not apply. Several inter-system gaps are not statistically significant (see text).}
...
\multirow{3}{*}{ATE (exact)}
  & Precision & 0.6138 & 0.6394 & 0.5348 & \textbf{0.6595} & 0.6510 \\
  & Recall    & \textbf{0.7143} & 0.6713 & 0.6921 & 0.6796 & 0.6546 \\
  & F1        & 0.6603 & 0.6549 & 0.6034 & \textbf{0.6694} & 0.6528 \\
\midrule
\multirow{3}{*}{ATE (partial)}
  & Precision & 0.7163 & 0.7411 & 0.6452 & \textbf{0.7833} & 0.7724 \\
  & Recall    & 0.8336 & 0.7781 & \textbf{0.8350} & 0.8072 & 0.7767 \\
  & F1        & 0.7705 & 0.7591 & 0.7279 & \textbf{0.7951} & 0.7746 \\
\midrule
\multirow{3}{*}{E2E Pair}
  & Precision & 0.5387 & \textbf{0.5667} & 0.4448 & 0.5532 & 0.5490 \\
  & Recall    & \textbf{0.6269} & 0.5950 & 0.5756 & 0.5700 & 0.5520 \\
  & F1        & 0.5795 & \textbf{0.5805} & 0.5018 & 0.5615 & 0.5505 \\
\midrule
\multirow{2}{*}{Sentiment}
  & Accuracy  & 0.8777 & \textbf{0.8864} & 0.8317 & 0.8388 & 0.8432 \\
  & Macro-F1  & 0.8113 & \textbf{0.8435} & 0.7717 & 0.7691 & 0.7673 \\
\midrule
...
\textbf{Qwen excels at aspect extraction among the LLMs.} Qwen~2.5-7B achieves the highest LLM ATE F1 on both exact (0.6603) and partial match (0.7705), driven by the best recall across all extraction metrics. It also attains a perfect 100\% JSON parse rate---a critical advantage for production annotation pipelines.

\textbf{Llama leads on sentiment and end-to-end performance.} Llama~3.1-8B obtains the highest E2E Pair F1 (0.5805), sentiment accuracy (0.8864), and sentiment macro-F1 (0.8435) of any system. Its precision-oriented extraction compensates for slightly lower recall, yielding the best aspect--polarity pair matching. Notably, the Pair F1 gap between Llama and Qwen is only 0.001---effectively a tie.
...
\textbf{Encoders are competitive at extraction; LLMs win on sentiment.} The fine-tuned Uzbek encoders are strong extraction baselines: BERTbek attains the best exact (0.6694) and partial (0.7951) ATE F1 of any system, despite having $\sim$1.5\% of the LLMs' parameters, and its exact-ATE margin over Qwen is small (+0.009). The LLM advantage concentrates elsewhere: sentiment macro-F1 (Llama 0.8435, Qwen 0.8113 vs.\ $\approx$0.77 for both encoders) and end-to-end pair F1 (0.5805/0.5795 vs.\ 0.5615/0.5505). A paired bootstrap (10{,}000 resamples) between the two encoders finds only their partial-match ATE difference significant ($p=0.023$); their exact-ATE, pair-F1, and sentiment differences are statistical ties. These results indicate that for pure span extraction in Uzbek, a laptop-trainable 110M encoder remains a highly competitive choice, whereas the generative LLMs deliver their value in joint aspect--sentiment prediction, categorical labeling within the same output structure, and the instruction-following flexibility that the annotation pipeline in \Cref{sec:meth_pipeline} depends on.
...
% ----------------------------------------------------------------------
\textbf{The Completeness Bottleneck}. Completeness (mean 3.32/5) is the weakest quality dimension: the model reliably extracts aspects present in the text but often misses domain-specific or subtle opinions, especially outside the restaurant domain. This is primarily due to the training data's narrow coverage---only five aspect categories---forcing the model to map unfamiliar aspects to the generic \textit{boshqalar} (miscellaneous) label, which dominates at 45.5\%. Addressing this bottleneck will require domain-adaptive fine-tuning or expanding the aspect taxonomy to better capture out-of-domain phenomena.

% ----------------------------------------------------------------------
\textbf{Checkpoint Effects}. R1 reasoning distillation conferred no advantage on structured ABSA in this setting: despite sharing the Qwen-7B backbone, the distilled checkpoint underperformed both Qwen and Llama, a pattern consistent with reasoning-oriented output habits sitting awkwardly against strict JSON formatting, though we did not isolate that mechanism. Llama achieves the lowest loss and best sentiment accuracy, but Qwen is superior for reliable extraction and instruction following (100\% parse rate, highest ATE recall). For annotation pipelines, robust JSON compliance and extraction coverage are more critical than marginal gains in sentiment metrics.

% ----------------------------------------------------------------------
\textbf{When is a 7B LLM Worth It?} The encoder baselines sharpen the practical picture. A 110M monolingual encoder, fine-tuned in ten minutes on a laptop, matches or exceeds the 7--8B LLMs at aspect-term extraction---a reminder that for span identification in a morphologically rich language, targeted monolingual pre-training (BERTbek, TahrirchiBERT) remains remarkably effective. The LLMs justify their cost where the task is genuinely joint and generative: sentiment macro-F1 (+4--7 points over the encoders), unified term--category--polarity output, and the ability to follow natural-language instructions on unseen domains, which is what the multi-domain annotation pipeline exploits. Teams building Uzbek ABSA systems under tight compute budgets should therefore consider encoders for extraction-only workloads and reserve LLMs for joint prediction and annotation-generation scenarios.
...
Three properties of the evaluation split warrant explicit caution. First, it functions as a development set---it is monitored during training and used for checkpoint inspection---and is then reused for the headline numbers, so the reported scores are optimistic relative to a genuinely untouched test set. Second, the split is drawn at random over annotation units without deduplication, and short formulaic reviews recur verbatim in this corpus; 61 of the 609 evaluation items (10.0\%) have an exact-text twin in training, overwhelmingly very short strings such as \textit{mazali} (``tasty'') or \textit{ajoyib} (``excellent''). The affected items are largely trivial ones on which all systems agree, and the contamination is identical for every system compared, so it does not favour any particular model; it does, however, inflate all absolute scores to an unknown degree. Third, no unadapted zero-shot or few-shot baseline is included, so the reported figures quantify what the fine-tuned systems achieve, not how much QLoRA itself contributed. We have since added grouped, deduplication-aware splitting to the released preprocessing code so that identical review texts cannot straddle the split; re-running the full comparison on independent train/development/test partitions, with zero-shot controls and multiple training seeds, is the primary methodological priority for follow-up work. Tokenizer efficiency and larger-scale, convention-reconciled human verification remain for future work.
...
This paper presented, to our knowledge, the first systematic study of parameter-efficient fine-tuning of open-source LLMs for Uzbek ABSA. We showed that QLoRA enables effective joint aspect extraction and sentiment classification via structured JSON generation, and that the fine-tuned LLMs are competitive with fine-tuned Uzbek BERT encoders. Our comparison revealed a clear architectural trade-off: Qwen~2.5-7B excels at reliable aspect extraction and instruction following, whereas Llama~3.1-8B achieves the highest precision for end-to-end sentiment matching, with the two effectively tied on end-to-end F1. We also found that validation loss is an unreliable proxy for downstream structured-extraction performance. Finally, we showed that a model trained only on restaurant reviews can produce usable annotations on more distant domains, though quality degrades measurably with domain distance. Our three-layer pipeline, together with a human-validation study of its output, produced a quality-scored multi-domain silver-standard resource with a gold-verified subset, offering a practical route to scalable low-resource dataset creation. Future work will focus on expanding the aspect taxonomy and on domain-adaptive fine-tuning to improve extraction completeness across diverse business domains.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\vspace{6pt} 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\authorcontributions{Conceptualization, S.M. and M.A.; methodology, S.M. and M.A.; software, S.M.; validation, S.M. and G.D.; formal analysis, S.M. and G.D.; investigation, S.M. and G.D.; resources, S.M.; data curation, S.M.; writing---original draft preparation, S.M.; writing---review and editing, S.M., G.D. and M.A.; visualization, S.M.; supervision, M.A.; project administration, S.M.; funding acquisition, not applicable. All authors have read and agreed to the published version of the manuscript.}
...
\dataavailability{The primary Uzbek aspect-based sentiment analysis dataset used for model fine-tuning is publicly available on Hugging Face at \url{https://huggingface.co/datasets/Sanatbek/aspect-based-sentiment-analysis-uzbek}. The multi-domain Uzbek business review corpus generated in this study, including the 80-review gold-verified subset with independent double annotation (\Cref{sec:res_human}), is publicly available on Zenodo at \url{https://zenodo.org/records/18790639} with DOI: \href{https://doi.org/10.5281/zenodo.18790639}{10.5281/zenodo.18790639}. Code for the encoder baselines, the human-validation analysis, and the significance tests is available in the project repository.
}
...
\acknowledgments{\label{sec:acknowledgement}We extend our heartfelt gratitude to Sardor Berdiyev, CEO and Co-founder of Commeta, for graciously granting permission to use publicly available reviews from the sharh.commeta.uz platform exclusively for academic research purposes. We thank Jaloliddin Rajabov for serving as the second native-speaker annotator in the human validation study (\Cref{sec:res_human}). We also acknowledge the computational resources provided by the National University of Uzbekistan. “During the preparation of this manuscript/study, the author Sanatbek used OpenAI's GPT model for the purposes of paraphrasing and better writing purposes. The authors have reviewed and edited the output and take full responsibility for the content of this publication.”}
...
\begin{table}[H]
\centering
\caption{LLM-as-Judge (GPT-4o-mini) quality scores for all 23 business domains, sorted by Overall score (descending). Stratified evaluation sample: 307 reviews. Scale: 1--5 (higher is better). $\dagger$~Small sample ($N \leq 5$): interpret with caution.\label{tab:domain_quality_full}}
...
\midrule
Bozor/BSC (Market/Shopping centre)$^\dagger$ & 3  & 3.67 & 5.00 & 5.00 & 5.00 & 4.67 \\
Sayohat/Turizm (Tourism/Travel)$^\dagger$   & 5  & 4.00 & 4.80 & 4.60 & 4.40 & 4.40 \\
Din/Madaniyat (Religion/Culture)$^\dagger$   & 3  & 4.33 & 5.00 & 4.00 & 5.00 & 4.33 \\
Yetkazib berish (Delivery)$^\dagger$          & 3  & 3.67 & 5.00 & 4.67 & 4.33 & 4.33 \\
Restoran/Ovqatlanish (Restaurant/Dining)      & 50 & 3.68 & 4.76 & 4.56 & 4.66 & 4.24 \\
Go'zallik (Beauty services)$^\dagger$         & 3  & 3.33 & 4.67 & 5.00 & 4.00 & 4.00 \\
...
Ko'ngilochar (Entertainment)$^\dagger$        & 3  & 3.33 & 4.67 & 4.33 & 4.00 & 3.67 \\
Transport/Yo'l (Transport/Roads)$^\dagger$    & 3  & 3.33 & 4.00 & 4.00 & 4.00 & 3.67 \\
...
\midrule
Davlat xizmatlari (Government services)$^\dagger$ & 3 & 3.00 & 4.00 & 4.33 & 3.67 & 3.33 \\
Telekommunikatsiya (Telecommunications)       & 25 & 3.04 & 4.36 & 3.64 & 3.64 & 3.32 \\
Ta'lim (Education)                            & 20 & 2.90 & 4.20 & 3.80 & 3.45 & 3.25 \\
Sport/Fitnes (Gyms/Fitness)$^\dagger$          & 4  & 2.75 & 4.25 & 4.25 & 3.00 & 3.25 \\
Kitob/Nashriyot (Books/Publishing)$^\dagger$  & 4  & 3.00 & 4.25 & 4.00 & 3.00 & 3.00 \\
Sug'urta (Insurance)$^\dagger$                & 3  & 2.33 & 3.67 & 2.67 & 3.33 & 2.67 \\
Investitsiya/Trading (Investment)$^\dagger$   & 3  & 2.33 & 4.00 & 3.67 & 3.00 & 2.67 \\
\bottomrule
\end{tabular}
\end{table}
