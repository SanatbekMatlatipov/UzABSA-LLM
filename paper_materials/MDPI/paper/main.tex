...
\abstract{In this paper we investigated the parameter-efficient fine-tuning of large language models (LLMs) for the ABSA task of Uzbek language. More specifically, we used Qwen~2.5-7B, Llama~3.1-8B, and DeepSeek-R1-Distill-7B open-source models under same QLoRA configurations to jointly extract aspect terms and classify its sentiment. To evaluate, we used 609  sample dataset which came from a review-grouped and deduplicated split. Before fine-tuning, we first evaluated the original LLMs using zero-shot prompting which served as controls for measuring the effect of adaptation. The results showed that the Qwen achieved the best aspect terms extraction of F1-score (0.708) with a 100\% parse rate, Llama leads in complete end-to-end ABSA task pair of F1-score (0.651) and sentiment accuracy (0.925). Comparing to unadapted zero-shot prompting, QLoRA fine-tuning at least doubles extraction of F1-score and triples pair F1-score, However, paired-bootstrap tests showed that the Qwen and Llama was not statistically significant. After that,the reason of benchmarking against fine-tuned Uzbek BERT  encoders containing 110M-parameter, shows a task-dependent trade-off which remained statistically indistinguishable from the LLMs at aspect extraction, whereas the LLMs are clearly stronger at sentiment and joint prediction. Based on these experiments, we proposed a three-layer model-assisted annotation pipeline application. In the first layer, local inference model that helped to generate candidate of aspect-sentiment annotations. In the second layer, LLM-as-Judge quality scoring  of the annotations. In the final layer, quality-tiered assembly of generated examples has been organized according to their scores. We then applied it to 5{,}038 business reviews from 23 domains, producing a multi-domain Uzbek ABSA dataset of 9{,}884 aspect based sentiment pairs. To ensure, a native-speaker validation study calibrated the LLM judge against human ratings with Spearman $\rho=0.66$ for overall quality and contributes an 80-review gold-verified subset. All code, models, and datasets are publicly released.}
