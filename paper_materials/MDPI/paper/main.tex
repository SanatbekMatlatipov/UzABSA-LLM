...
\abstract{Uzbek, a Turkic language with almost 38 million speakers, remains under-served in aspect-based sentiment analysis (ABSA). We present, to our knowledge, the first systematic study of parameter-efficient fine-tuning of open-weight large language models (LLMs) for Uzbek ABSA: Qwen~2.5-7B, Llama~3.1-8B, and DeepSeek-R1-Distill-7B are fine-tuned under identical QLoRA configurations to jointly extract aspect terms and classify sentiment as structured JSON. On a 609-example evaluation set drawn from a review-grouped, deduplicated split, Qwen achieves the best extraction F1 (0.708) with a 100\% parse rate, while Llama leads in end-to-end pair F1 (0.651) and sentiment accuracy (0.925). Relative to unadapted zero-shot controls, QLoRA fine-tuning at least doubles extraction F1 and triples pair F1, and paired-bootstrap tests show the two leading LLMs are statistically tied. Benchmarking against fine-tuned Uzbek BERT encoders reveals a task-dependent trade-off: 110M-parameter encoders remain statistically indistinguishable from the LLMs at span extraction, whereas the LLMs are clearly stronger at sentiment and joint prediction. We then propose a three-layer model-assisted annotation pipeline---local inference, LLM-as-Judge quality scoring, and quality-tiered assembly---and apply it to 5{,}038 business reviews from 23 domains, producing a multi-domain Uzbek ABSA dataset of 9{,}884 aspect--sentiment pairs. A native-speaker validation study calibrates the LLM judge against human ratings (Spearman $\rho=0.66$ for overall quality) and contributes an 80-review gold-verified subset. All code, models, and datasets are publicly released.}
...
Beyond the language-specific results, the study addresses a problem generic to big-data text analytics: turning a heterogeneous, continuously growing stream of user-generated reviews into a quality-controlled analytical resource when expert annotation cannot scale with the data. Review platforms accumulate text far faster than annotators can label it, and in a low-resource language there is no pool of annotators to draw on in the first place---so the binding constraint on analysing this data is annotation throughput, not storage or compute. Our answer couples a locally fine-tuned annotator with an automated cognitive evaluation layer---an LLM judge whose scores we calibrate against native-speaker judgement---and a tiered assembly step that makes annotation quality explicit, per instance, in the released data. The pipeline is model-agnostic and transfers to other low-resource languages and domains where the same annotation bottleneck applies.
...
\midrule
Compute time (min)       & ---$^{\dagger}$   & ---$^{\dagger}$   & 84   \\
